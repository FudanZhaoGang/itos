...
def load_intent(intentpath=r"E:\retrieval-engine\sales\data\corpus1.0\intent_list_corpus1.0.json"):
    fjson = open(intentpath, "r", encoding="utf-8")
    intent = json.load(fjson)
    return intent


def load_pretrained_model(pretrained_path=r'E:/retrieval-engine/sales/chinese-bert/',
...
def load_finetune_model(finetune_path=r"E:\retrieval-engine\sales\save\bert-finetune-crossencoder-02.pt",
                        pretrained_path=r'E:/retrieval-engine/sales/chinese-bert/',
...
def demo(user_text, intent):
    ...
    cosine_similarity = F.cosine_similarity(query_sentence_embedding.squeeze(0), intent_sentence_embedding.squeeze(0),
                                            dim=0)
    print("Intent = {}, Score = {}".format(intent, score))
    print("Intent = {}, Score = {}".format(intent, cosine_similarity))


def filter(query, index_list, tokenizer, model, topN=-1):
    _score = 0
    target_index = -1
    ...
    if topN > 0:
        topN = min(topN, len(intent_list))
        topN_list, topN_indices = torch.topk(all_score, k=topN)

        topN_score = [topN_list[i].item() for i in range(topN)]
        topN_index = [index_list[topN_indices[i]] for i in range(topN)]
        topN_intent = [intent_list[topN_indices[i]] for i in range(topN)]

        return topN_score, topN_index, topN_intent
